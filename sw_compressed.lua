--compressed and unreadable version for pasting
str=string
scr=screen
...
local tmp=''
while num>0 do
tmp=string.sub(a,num%2+1,num%2+1)..tmp
...
function drawTextBoxS(x,y,w,h,text,h_a,v_a,sc)
x=x+(w-str.len(text)*sc*9)*((h_a+1)/2)
y=y+(h-10*sc)*((v_a+1)/2)
for k=1,str.len(text) do
for y1=0,9 do
for x1=0,7 do
if str.sub(s[str.lower(str.sub(text,k,k))],y1*8+x1+1,y1*8+x1+1) == '1' then
scr.drawRectF(x+x1*sc+(k-1)*9*sc,y+y1*sc,sc,sc)
end
end
end
end
...
function onDraw()
	drawTextBoxS(0,0,96,32,'Abc',0,0,1)
end
